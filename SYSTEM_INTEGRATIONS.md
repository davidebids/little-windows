...
7. Use CloudKit Dashboard to inspect the development schema and deploy it to production before TestFlight/App Store use.
8. For Family Sync testing, create a share from Settings > Family Sync on the owner's device, accept the iCloud invitation on a second Apple Account, and verify both devices can write care data.
