...
When Family Sync is enabled, SwiftData remains the local cache and `CloudKitSharingService` synchronizes versioned `FamilyEntity` records in one shared CloudKit zone. Household records and only profiles explicitly set to **Share this profile with Family Sync** are exported; private profiles and their care events, appointments, milestones, medications, dose records, health observations, photos, guide state, and routines are excluded. Importing a shared refresh preserves those private profiles locally. Each included object has an independent asset record plus an update timestamp; deletions are uploaded as tombstones. A lightweight root record carries the dataset checksum and schema version. Accepted caregivers read and write the same shared data, while widgets and Live Activities continue to refresh from each device's local cache.
