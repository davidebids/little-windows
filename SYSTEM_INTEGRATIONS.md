...
Medication notifications carry only the identifiers and entered dose facts needed to reopen the app. Taken and Skipped actions are revalidated against the current active schedule before `MedicationService` applies them in the main app, so a delivered notification from an edited schedule cannot create a stale dose. Editing a medication retires the prior regimen, preserves its phases for historical dose meaning, creates a new active version, and cancels requests tied to the retired regimen before foreground reconciliation schedules the replacement. Dose identity, the care timeline, supply history, plan audit history, review status, profile scope, and Family Sync conflict handling remain consistent. Taken-dose history drives a confidence-labeled supply run-out estimate and trip overlap warnings. Prescription metadata drives one open refill task per medication, with optional due date and caregiver assignment; task pickup records a refill supply adjustment and decrements remaining refills. Closing a refill task clears its collaboration state, and archiving its medication cancels the task. Foreground reconciliation shares a bounded rolling notification budget across all active medication schedules after accounting for pending non-medication alerts; only requests accepted by iOS consume that budget. An active snooze survives unrelated reconciliation only while its occurrence is still valid and unlogged. Logging the dose, editing its schedule, or archiving the medication removes the initial, follow-up, and snoozed requests.
...
For children 6–12 months, Feeding balance reviews a seven-day solids window using the caregiver's stated logging coverage. From four through five months it can surface only after at least four logged meals across two days show the same food in at least 60% of meals; its copy continues to recommend starting at about six months based on readiness and clinician guidance. It can surface quiet variety, produce/bean, iron-rich-food, and repeated-food patterns; missing logs remain unknown rather than being treated as a deficiency. Child diaper logs can record hard stool, difficult or painful passage, prolonged straining, and visible blood, with an explicit option to link the observation to Feeding balance. A causality-neutral timeline places recent stool observations beside solids, health, medicine, and caregiver context. Caregiver-recorded concerns, resolution state, optional follow-up time, and logging coverage live on `SolidsProfileState`, are included in JSON backup and Family Sync, and use non-diagnostic safety copy. Restored check-in history is normalized newest-first with one current concern; editing or linking older history cannot replace that concern or cancel its follow-up. A new observation preserves a still-pending follow-up for the ongoing concern, while expired or orphaned reminder state is discarded. Red-flag concerns suppress ordinary food and recipe suggestions. Every food detail exposes age-aware digestive notes; food-specific constipation cautions are limited to explicitly sourced contexts.

When Family Sync is enabled, SwiftData remains the local cache and `CloudKitSharingService` synchronizes versioned `FamilyEntity` records in one shared CloudKit zone. Household records and only profiles explicitly set to **Share this profile with Family Sync** are exported; private profiles and their care events, appointments, structured appointment follow-ups, milestones, medications, medication plan revisions and reconciliation sessions, medication refill tasks, dose records, health observations, photos, guide state, routines, and attention collaboration records are excluded. Importing a shared refresh preserves those private profiles locally. Each included object has an independent asset record plus an update timestamp; deletions are uploaded as tombstones. A lightweight root record carries the dataset checksum and schema version. Accepted caregivers read and write the same shared data, while widgets and Live Activities continue to refresh from each device's local cache. Accepting an invitation shows persistent in-app progress while iCloud accepts the share and downloads its data, followed by an explicit success or failure result; a new empty install does not cover that progress with onboarding.
...
Local Family Sync mutations are serialized so newer timer states cannot be overtaken by an older upload. Synchronization uses a saved common baseline and a three-way, per-record merge, so offline changes to different objects are preserved and the newest `updatedAt` wins when two devices edit the same object. Initial share creation normally uploads one baseline snapshot; `FamilyEntity` assets then store only changed, new, or deleted records and are overlaid on that snapshot when another caregiver syncs. A snapshot above the safe CloudKit asset size falls back to small record- and byte-limited entity batches. Creation reports progress in Settings, applies explicit request/resource time limits, and removes its newly created zone if setup fails. Push-subscription verification continues after the invitation sheet is ready and is retried by launch/manual sync, so a slow push setup cannot block inviting a caregiver. Automatic launch and silent-push checks compare lightweight root metadata before querying entities. Share-sheet membership callbacks are debounced, overlapping membership fetches are coalesced, and the accepted-caregiver directory is rewritten only when its contents change. Local mutations queue their CloudKit upload immediately. While the app is active in shared mode, it performs an immediate metadata check and repeats every second as a fallback for delayed CloudKit pushes. Foreground checks and transient local-upload failures retry with a short exponential backoff, while overlapping sync work remains serialized.

Family Sync schema version 5 requires the `FamilyEntity` record type to be created in the CloudKit development environment with `collection` (String), `entityID` (String), `payloadAsset` (Asset), `payloadChecksum` (String), `isDeleted` (Int(64)), and `entityUpdatedAt` (Date/Time), then deployed to production before a release build is distributed. Pre-release version-1 through version-4 shares should be stopped and recreated; there is intentionally no legacy share migration because the app has no production users. Private SwiftData CloudKit schema initialization must also include `CD_CareProfile`, `CD_CareEvent`, `CD_Medication`, `CD_MedicationRegimen`, `CD_MedicationSchedulePhase`, `CD_MedicationPlanRevision`, `CD_MedicationReconciliation`, `CD_MedicationRefillTask`, `CD_MedicationDoseRecord`, `CD_MedicationSupplyLog`, `CD_DoctorAppointment`, `CD_AppointmentFollowUp`, `CD_HouseholdAttentionAcknowledgement`, `CD_HouseholdAttentionClaim`, `CD_CaregiverHandoffNote`, `CD_FamilyCaregiverIdentity`, `CD_SolidFoodCatalogItem`, `CD_SolidsProfileState`, `CD_SolidFoodProgress`, `CD_SolidFoodEventItem`, `CD_SolidAllergenProgress`, `CD_PlannedSolidMeal`, `CD_PackingTrip`, `CD_TripTraveler`, `CD_PackingBag`, `CD_PackingItem`, `CD_TripItineraryChoiceGroup`, `CD_TripItineraryItem`, and `CD_TripItineraryLink`; `CD_CareProfile` includes optional `CD_birthDate`, `CD_adultRelationshipRawValue`, `CD_sharingScopeRawValue`, and `CD_ownerIdentifier`; `CD_CareEvent` includes `CD_diaperRash`, `CD_solidFoodDetailsJSON`, and `CD_healthObservationDetailsData`; `CD_Medication` includes optional `CD_lastReviewedAt`, `CD_isConfirmedCurrent`, `CD_refillLeadDays`, `CD_prescriptionNumber`, `CD_fillQuantity`, `CD_refillsRemaining`, and `CD_prescriptionExpirationDate`; `CD_MedicationRegimen` includes `CD_followUpRemindersEnabled`; `CD_MedicationDoseRecord` includes `CD_actualDoseAmount`, `CD_timingRawValue`, `CD_reasonRawValue`, and `CD_updatedAt`; `CD_MedicationSupplyLog` includes optional `CD_doseRecordID`; the existing refill-task, solids, Food, Home, Trips, appointment, and attention fields must also remain present. Deploy the complete development schema before release.

`CD_CareEvent` now also includes optional `CD_pooDifficultOrPainful`, `CD_pooProlongedStraining`, `CD_pooVisibleBlood`, and `CD_linksDigestiveConcern`. `CD_SolidsProfileState` now also includes `CD_digestiveCheckInsJSON`, `CD_digestiveLoggingCoverageRawValue`, `CD_digestiveReminderEnabled`, and optional `CD_digestiveReminderAt`; initialize and deploy these development-schema fields before distributing this feature in a signed CloudKit build.
...
littlewindows://care/solids/digestive
...
Digestive-check-in follow-ups use the same one-shot scheduling policy and open `littlewindows://care/solids/digestive`. A concern created directly or linked from a diaper log can add or cancel its next-day follow-up from Feeding balance. Resolving or removing the active concern cancels its pending reminder, and central reconciliation removes orphaned `solids.digestive.*` requests.
...
31. With a six-month-or-older child selected, open Solids and verify the 535-food database, stage-based first-100 path, seven-day guided schedule, swap/shift controls, 424-recipe library, recipe filters and named lists, food-to-recipe links, technique photography and auto-playing preparation walkthroughs, meal plan reminders, inventory-aware shopping handoff, tracker filters, and per-food chronological history. Log two foods in one meal with a serving amount and preference on each and a suspected reaction on only one; verify the meal detail and allergen screen keep amount, symptoms, severity, timing, response, and follow-up attached to the correct food. From an allergen page, plan the remaining introduction portions, verify the precise step and serving guidance reach the log editor, and confirm only explicitly checked portions advance the three-step introduction. Complete three separate tolerated exposures and verify unique-meal counts, safe recipe suggestions, rotation planning, and the Today follow-up prompt. Open **View feeding report** and verify Reports -> Summary -> Feeding shows period-correct solid meals, unique foods, new foods, allergen exposures, and reaction notes; verify its tracker/allergen links and that tapping a solid event in Day/List returns to solid meal detail. Create a custom food with age, preparation, safety, allergen, and photo metadata; mark it want-to-try/favorite, plan it, add it to shopping and inventory flows, log its allergen metadata, reuse it, delete the catalog entry, and confirm its historical timeline remains accessible. Verify JSON backup/import plus Family Sync preserve all solids data and named recipe lists.
   - In **Feeding balance**, set logging coverage, log several meals across two days, and confirm the seven-day review distinguishes partial logs from useful patterns. At five months, confirm the same food in at least 60% of four meals across two days surfaces only the conservative early-pattern review and pre-six-month safety copy. Log a poo diaper with stool consistency, comfort, straining, and blood observations; explicitly link it to Feeding balance, then edit, unlink, and delete it and confirm the mirrored concern follows those changes. Confirm the seven-day **What changed?** timeline combines stool, solids, medicine, health, and caregiver context without claiming causation. Verify red-flag symptoms hide ordinary food suggestions. Schedule a next-day follow-up from both a direct and diaper-linked concern, open the notification into Feeding balance, then resolve the concern and confirm its reminder is removed. Open built-in and custom foods and verify every one has age-aware digestive notes and only source-backed food-specific cautions. Confirm suggested foods and recipes open their existing detail and shopping-list flows.
...
35. Add an adult profile for yourself and another for a loved one, leaving both private. Verify adult Today and widget actions, appointments, medication schedules, versioned medication edits, effective dates and sources, before/after audit history, reconciliation from an appointment and discharge paperwork, confirmed-current and last-reviewed status, as-needed guardrails, dose corrections/deletion, dose and supply history, actual-use run-out estimates, prescription and refill fields, refill request/ready/pickup states, pickup supply adjustment, symptoms, core vitals, adult Reports trends, the exported current medication plan, and adult milestone suggestions. Create a trip spanning the estimated run-out date and confirm both the medication and trip detail screens warn about it. Confirm an old dose remains linked to its retired regimen after a plan change. Confirm child sleep predictions, pediatric growth references, solids, and age guides do not appear for either adult.
...
37. With three Family Sync caregivers and at least one private profile, create structured appointment follow-ups plus an assigned medication refill, due medication, routine, low-stock, return, trip, planned-solids, and allergen items. Verify Today > Home keeps one Needs Attention section, shows the highest-priority five with View all, and labels each row with its source, profile or household scope, due state, and responsibility. Have one caregiver mark a follow-up seen without assigning it, then assign and reassign it between the other two caregivers; repeat assignment and progression through request, ready, and pickup for the refill task, verifying every device shows exactly one responsible caregiver and the updated state. Edit an assigned follow-up without changing its caregiver and confirm the assignment remains intact. Add at least four handoff notes, including one from the current caregiver, and verify the newest three appear on the card while View all opens the complete timestamped history. Confirm only shared-profile or household activity appears under Since you last checked, Needs your acknowledgement, and Next up for you; private and legacy profile-less appointments must have no acknowledgement, assignment, or handoff controls. Leave Home before scrolling to Handoff and confirm new activity remains unread, then view the Handoff card and confirm the checkpoint advances for the next visit. Snooze an item on one device and verify it moves to the local Snoozed section with its return time, remains visible in Needs Attention on the other devices, and can be restored immediately with Unsnooze. Let another snooze expire and confirm it returns automatically, then complete it from its source and verify the queue updates everywhere.
